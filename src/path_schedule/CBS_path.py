...
class Solution:
    def __init__(self, dimension, agents, a_star_max_iter=-1):
    ...
    def get_first_conflict(self, solution):
        ...
        for t in range(max_t):
            ...
            for s in solution.values():
                if t in s.keys():
                    all_car_position.append([s[t].location.x, s[t].location.y])
                else:
                    all_car_position.append([list(s.values())[-1].location.x, list(s.values())[-1].location.y])
            for idx1, car1 in enumerate(all_car_position):
                for idx2, car2 in enumerate(all_car_position):
                    if idx1 == idx2: continue
                    if (car1[0]-car2[0])**2 + (car1[1]-car2[1])**2 < 10:
                        result.time = t
                        result.type = Conflict.VERTEX
                        result.location_1 = solution[t].coord
                        result.agent_1 = list(solution.keys())[t]
                        result.agent_2 = list(solution.keys())[t]
                        return result
            for car in cars:
    ...
    def state_valid(self, state):
        print(np.shape(self.dimension))
        print(state.location.y-421)
        print(state.location.x-181)
        return not self.dimension[state.location.y-421][state.location.x-181]\
    ...
    def compute_solution(self, task_agent):
        solution = {}
...
class AStar:
    ...
    def reconstruct_path(self, came_from, current):
        ...
        t = 0
        for i in total_path[::-1]:
            total_path_dict[t] = i
            t+=1
        return total_path_dict

    def search(self, task_agent):
        """
        low level search
        """
        initial_state = self.agent_dict[task_agent]["start"]
        step_cost = 1
        ...
        f_score = {}
        h_score = self.admissible_heuristic(initial_state, self.agent_dict[task_agent]["goal"])
        ...
        while open_set and (self.max_iter == -1 or self.iter < self.max_iter):
            ...
            if self.is_at_goal(current, self.agent_dict[task_agent]["goal"]):
            ...
            for neighbor in neighbor_list:
                ...
                h_score = self.admissible_heuristic(neighbor, self.agent_dict[task_agent]["goal"])
...
def CBS(task_cars, grid):
    ...
    Path_solution = Solution(grid, task_cars)
    start = HighLevelNode()
    start.constraint_dict = {}
    for agent in Path_solution.agent_dict.keys():
        start.constraint_dict[agent] = Constraints()
    start.solution = Path_solution.compute_solution(task_cars)
    ...
    while open_set:
        ...
        conflict_dict = Path_solution.get_first_conflict(P.solution)
        ...
        for agent in constraint_dict.keys():
            ...
            new_node.solution = Path_solution.compute_solution()
...
if __name__ == "__main__":
    ...
    for i in range(432, 435):
        for j in range(186, 197):
            grid[i-421][j-181] = 1
    car1 = {"name": "001", "start": [190, 425], "goal": [190, 444]}
    car2 = {"name": "002", "start": [186, 430], "goal": [199, 431]}
    all_car = [car1, car2]
    CBS(all_car, grid)