...
class Solution:
    def __init__(self, dimension, task_agent, agents, a_star_max_iter=-1):
    ...
    def get_neighbors(self, state):
        ...
        if self.state_valid(n):
            neighbors.append(n)
        # Up action
        n = State(state.time + 1, Location(state.location.x, state.location.y+3))
        ...
        if self.state_valid(n) and self.transition_valid(state, n):
            neighbors.append(n)
        # Left action
        n = State(state.time + 1, Location(state.location.x-3, state.location.y))
        if self.state_valid(n) and self.transition_valid(state, n):
            neighbors.append(n)
        # Right action
        n = State(state.time + 1, Location(state.location.x+3, state.location.y))
    ...
    def get_first_conflict(self, task_car, solution, cars):
        max_t = len(solution.keys())
        result = Conflict()
    ...
    def compute_solution(self, task_agent, target_node):
        solution = {}
        self.constraints = self.constraint_dict.setdefault(task_agent, Constraints())
        local_solution = self.a_star.search(task_agent, target_node)
...
class AStar:
    def __init__(self, env):
        def admissible_heuristic(self, state, target_point):
    ...
    def search(self, task_agent, target_point):
        """
        low level search
        """
        initial_state = task_agent.location
        step_cost = 1
...
def CBS(task_car, current_point, target_point, env):
    ...
    Path_solution = Solution([(180, 420), (200, 450)], task_car, env.cars)
    start = HighLevelNode()
    start.constraint_dict = {}
    for agent in Path_solution.agent_dict.keys():
        start.constraint_dict[agent] = Constraints()
    start.solution = Path_solution.compute_solution(task_car, target_point)
