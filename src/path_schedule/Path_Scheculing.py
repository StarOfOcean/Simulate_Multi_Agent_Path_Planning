...
class Solution:
    def __init__(self, dimension, task_car, current_point, target_point, a_star_max_iter=200):
    ...
    def get_first_conflict(self, task_car, solution, cars, T):
        max_t = len(solution)
        for car in cars:
            if list(car.path_dict.keys())[-1]-T > max_t:
                max_t = int(list(car.path_dict.keys())[-1]-T)
        result = Conflict()
        for t in range(max_t):
            for car in cars:
                ...
                if t >= len(solution):
                    task_car_position = (solution[-1].location.x, solution[-1].location.y)
                else:
                    task_car_position = (solution[t].location.x, solution[t].location.y)
                if (task_car_position[0]-car_position[0])**2+(task_car_position[1]-car_position[1])**2 < 10:
                    if t >= len(solution):
                        t = len(solution)-1
                    result.time = t
